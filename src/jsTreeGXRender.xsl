...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'jsTreeGX'">
        <xsl:call-template name="RenderjsTreeGX"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- jsTreeGX design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderjsTreeGX">
          ...
          <TR>
            <img style="background-repeat: no-repeat;" padding="0">
              <xsl:attribute name ="src">
                ...
                <xsl:text>\resources\jstree\jstree.png</xsl:text>
              </xsl:attribute>
            </img>
          </TR>
        </TBody>
